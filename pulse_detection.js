...
var constraints = { video:{ 
  facingMode: "user",
  frameRate: { min: 30, ideal: 30 }, //Adjust ideal to 60 to increase max frame rate
  exposureMode: "none",
...
function cameraStart() {
  navigator.mediaDevices
    ...
    //frameRateSet(FPS);
    FrameRate = setInterval(ImStream, ((1/FPS)*1000));
  ...
  if (Fin == 0){
    ...
    if (Finger >= Math.round((5)*FPS) && Finger<Math.round((37))*FPS){
      ...
      if (Finger >= (Math.round((4)*FPS))+1){
        ...
        ctxWave.moveTo(x_1, y_1);    
        ctxWave.lineTo(x_2, y_2);  
        ctxWave.stroke();
